...
lines=['# Top 50 Box agent use cases','', 'Editorial build-priority ranking for Box for Muse and reusable Box agent tools. These are our proposals, not a list authored or endorsed by Aaron Levie. All are proposed, not implemented. Sources and limitations: [research notes](research/SOURCES.md).','', 'Ranking favors useful content outcomes, reuse of core primitives, testability, and manageable permissions. Adjacent ranks are judgment calls, not measured ROI. Core is a dependency category, not a shipping claim.','', '| Rank | Use case | Trigger → result | Essential boundary | Gate | Theme |','| --- | --- | --- | --- | --- | --- |']
